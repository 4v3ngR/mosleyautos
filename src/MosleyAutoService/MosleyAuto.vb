    ...
    Private Sub MosleyAuto_Tick(sender As Object, e As EventArgs) Handles Me.Tick
        Try
            ...
            If GetInteriorID(ply.Position) = mosleyIntID AndAlso Not unWelcome.Contains(veh.ClassType) Then
                Try
                    GTA.Native.Function.Call(GTA.Native.Hash.SET_INTERIOR_ACTIVE, mosleyIntID, True)
                Catch
                End Try

                If Not isExiting AndAlso Not TestDrive = 3 Then
                    ' -22.02124, -1678.407, 28.89593
                    If veh.Position.DistanceTo(New Vector3(-15.5, -1670.0, 28.9)) <= 3 Then
                        PlayerVehicleHalt()
                        UpdateTitleName()
                        PlayEnterCutScene()
                        PutVehIntoShop()
                    Else
                        If veh.Position.DistanceTo(New Vector3(-11.1047, -1668.97, 28.90443)) <= 1 Then
